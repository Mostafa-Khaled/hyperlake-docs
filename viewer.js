...
async function getPage(product, route) {
  ...
  if (!loaded.has(ref.chunk)) {
    loaded.set(ref.chunk, await fetch(`${getBasePath()}/data/${ref.chunk}?v=1.0.7`).then(r => r.ok ? r.json() : []));
  }
...
async function loadAllProductPages(product, onProgress) {
  ...
  await Promise.all(chunks.map(async chunk => {
    if (!loaded.has(chunk)) {
      const data = await fetch(`${getBasePath()}/data/${chunk}?v=1.0.7`).then(r => r.ok ? r.json() : []);
...
manifest = await fetch(`${getBasePath()}/data/manifest.json?v=1.0.7`).then(r => r.json());
initProductPicker();
renderRoute();
